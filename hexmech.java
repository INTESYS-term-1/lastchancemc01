...
*****************************************************************************/
	public static void fillHex(int i, int j, String n, Graphics2D g2) {
		String c;
		int x = i * (s+t);
		int y = j * h + (i%2) * h/2;
		if (hexgame.board[i][j].getOwner() == -1) {
		...
		else if(hexgame.board[i][j].getOwner() == 1){
		...
		else if(hexgame.board[i][j].getOwner() == 0){
